...
def evaluation_config():
    ...
    return {
        ...
        "metrics": ["answer_accuracy", "faithfulness", "context_precision", "context_recall"]
    }
...
def test_evaluator_setup(mock_evaluate, mock_cr, mock_cp, mock_f, mock_ac,
...
def test_evaluator_evaluate(mock_dataset, mock_evaluate, mock_cr, mock_cp, mock_f, mock_ac,
    ...
    with patch.object(evaluator, 'setup'):
        ...
        evaluator.metrics = ["answer_correctness_metric", "faithfulness_metric", "context_precision_metric", "context_recall_metric"]
    ...
    assert len(results["average_scores"]) == 4

    # Make sure the average scores are calculated correctly
    assert results["average_scores"]["answer_correctness"] == 0.775  # (0.7 + 0.85) / 2
